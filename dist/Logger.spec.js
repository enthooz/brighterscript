...
describe('Logger', () => {
    ...
    it('loglevel setter converts string to enum', () => {
        ...
        (0, chai_config_spec_1.expect)(logger.logLevel).to.eql(Logger_1.LogLevel.error);
        logger.logLevel = 'info';
        (0, chai_config_spec_1.expect)(logger.logLevel).to.eql(Logger_1.LogLevel.info);
    });
    it('uses LogLevel.log by default', () => {
        logger = new Logger_1.Logger();
        (0, chai_config_spec_1.expect)(logger.logLevel).to.eql(Logger_1.LogLevel.log);
    });
    describe('log methods call correct error type', () => {
        it('error', () => {
            const stub = sinon.stub(logger, 'writeToLog').callsFake(() => { });
            logger.error();
            (0, chai_config_spec_1.expect)(stub.getCalls()[0].args[0]).to.eql(console.error);
        });
        it('warn', () => {
            const stub = sinon.stub(logger, 'writeToLog').callsFake(() => { });
            logger.warn();
            (0, chai_config_spec_1.expect)(stub.getCalls()[0].args[0]).to.eql(console.warn);
        });
        it('log', () => {
            const stub = sinon.stub(logger, 'writeToLog').callsFake(() => { });
            logger.log();
            (0, chai_config_spec_1.expect)(stub.getCalls()[0].args[0]).to.eql(console.log);
        });
        it('info', () => {
            const stub = sinon.stub(logger, 'writeToLog').callsFake(() => { });
            logger.info();
            (0, chai_config_spec_1.expect)(stub.getCalls()[0].args[0]).to.eql(console.info);
        });
        it('debug', () => {
            const stub = sinon.stub(logger, 'writeToLog').callsFake(() => { });
            logger.debug();
            (0, chai_config_spec_1.expect)(stub.getCalls()[0].args[0]).to.eql(console.debug);
        });
        it('trace', () => {
            const stub = sinon.stub(logger, 'writeToLog').callsFake(() => { });
            logger.trace();
            (0, chai_config_spec_1.expect)(stub.getCalls()[0].args[0]).to.eql(console.trace);
        });
    });
    ...
    describe('time', () => {
        ...
        it('returns value', () => {
            ...
            (0, chai_config_spec_1.expect)(logger.time(Logger_1.LogLevel.log, null, spy)).to.be.true;
            (0, chai_config_spec_1.expect)(spy.called).to.be.true;
        });
        ...
        it('waits for and returns a promise when a promise is returned from the action', () => {
            (0, chai_config_spec_1.expect)(logger.time(Logger_1.LogLevel.info, ['message'], () => {
